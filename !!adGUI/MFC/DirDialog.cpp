...
// The normal edit control (cmb13) where the file name is entered is hidden and replaced 
...
// the contents of the hidden edit control (cmb13) and simulating a click of the hidden
...
// directory name into the original edit control (cmb13) and simulating a click of the
...
void CDlgWnd::CheckDir(const CString &ss)
{
    ...
    CEdit *pOld = (CEdit *)GetDlgItem(cmb13);
    ASSERT(pOld != NULL);
...
void CDirDialog::OnInitDone()
{
    ...
    // Create a new edit control where cmb13 now is
    ASSERT(pp->GetDlgItem(cmb13) != NULL);
    pp->GetDlgItem(cmb13)->GetWindowRect(rct); //Get cmb13 rectangle
    ...
    m_Edit.SetFont(pp->GetDlgItem(cmb13)->GetFont());
...
//  m_Edit.SetSel(0, strlen(m_ofn.lpstrInitialDir));
    ...
    ASSERT(pp->GetDlgItem(cmb13) != NULL);
    pp->GetDlgItem(cmb13)->GetWindowRect(rct);
    pp->ScreenToClient(rct);
