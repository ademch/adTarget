...
OpenGLSubWindowWithGUI::OpenGLSubWindowWithGUI(int iBottomLeftX, int iBottomLeftY, int iWidth, int iHeight) :
...
bool OpenGLSubWindowWithGUI::MouseHWheelFunc(int state, int delta, int x, int y)
{
	...
	if ((x > m_iBottomLeftX) && (x < m_iBottomLeftX + m_iWidth) &&
		(y > m_iBottomLeftY) && (y < m_iBottomLeftY + m_iHeight))
	{
		if (MouseHWheelFuncGUI(state, delta, x, y)) return true;
	}
...
bool OpenGLSubWindowWithGUI::MouseHWheelFuncGUI(int state, int delta, int x, int y)
{
	SetupGraphicsPipelineWithIdentityModelViewMatrix();
