...
: m_sFile(_T("Untitled.avi")), m_dwRate(30),m_pAVIFile(NULL), m_pStream(NULL), m_pStreamCompressed(NULL)
{
...
: m_sFile(sFileName), m_dwRate(dwRate), m_pAVIFile(NULL), m_pStream(NULL), m_pStreamCompressed(NULL)
{
		MakeExtAvi();
...
HRESULT CAVIGenerator::InitEngine()
{
	...
	//Step 0  : clear file if exists
	
	out=fopen("Clip.avi","w+");
	if (out)
	{	fclose(out);
		unlink("Clip.avi");
	}
	...
	fcc[4]=0;
	fcc[0]= opts.fccHandler;
	fcc[1]= opts.fccHandler >> 8;
	fcc[2]= opts.fccHandler >> 16;
	fcc[3]= opts.fccHandler >> 24;
	...
	// Step 6 : Initialize frame counter
	m_lFrame=0;
