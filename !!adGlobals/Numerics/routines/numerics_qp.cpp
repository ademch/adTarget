...
void compute_d(dvector& d, const dmatrix& J, const dvector& np);
void update_z(dvector& z, const dmatrix& J, const dvector& d, int iq);
void update_r(const dmatrix& R, dvector& r, const dvector& d, int iq);
bool add_constraint(dmatrix& R, dmatrix& J, dvector& d, int& iq, double& rnorm);
void delete_constraint(dmatrix& R, dmatrix& J, ivector& A, dvector& u, int n, int p, int& iq, int l);

// Utility functions for computing the Cholesky decomposition and solving linear systems
void cholesky_decomposition(dmatrix& A);
void cholesky_solve(const dmatrix& L, dvector& x, const dvector& b);
void forward_elimination(const dmatrix& L, dvector& y, const dvector& b);
void backward_elimination(const dmatrix& U, dvector& x, const dvector& y);

// Utility functions for computing the scalar product and the euclidean 
// distance between two numbers
double scalar_product(const dvector& x, const dvector& y);
	...
	dvector& x)
{
	...
	if (G.nrows()!=n || CE.nrows()!=n || ce0.size()!=p || CI.nrows()!=n || ci0.size()!=m)
	...
	dmatrix R(n, n), J(n, n);
	dvector s(m + p), z(n), r(m + p), d(n), np(n), u(m + p), x_old(n), u_old(m + p);
	...
	ivector A(m + p), A_old(m + p), iai(m + p);
	int q, iq, iter = 0;
...
inline void compute_d(dvector& d, const dmatrix& J, const dvector& np)
{
...
inline void update_z(dvector& z, const dmatrix& J, const dvector& d, int iq)
{
...
inline void update_r(const dmatrix& R, dvector& r, const dvector& d, int iq)
{
	register int i, j;/*, n = d.size();*/
	register double sum;
...
bool add_constraint(dmatrix& R, dmatrix& J, dvector& d, int& iq, double& R_norm)
{
	int n = d.size();
...
inline double scalar_product(const dvector& x, const dvector& y)
{
...
inline void forward_elimination(const dmatrix& L, dvector& y, const dvector& b)
{
...
inline void backward_elimination(const dmatrix& U, dvector& x, const dvector& y)
{
...
int solve_quadprog_disregard(int a, int b) { return 7; }
