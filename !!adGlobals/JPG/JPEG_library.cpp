...
// include jdatadst.c(rename to cpp) and jdatasrc.c(rename to cpp) into this projet
					...
					/*out*/unsigned int& imageHeight )
{
	...
	*/
	...
	(*dataImg) = (unsigned char*)malloc(imageWidth * imageHeight * cinfo.output_components);
	unsigned char* dataDst = (*dataImg);
	...
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		memcpy(dataDst, buffer[0], row_stride);
		dataDst += row_stride;
	}

	// lodepng library returns dirty alpha
	dataDst = (*dataImg);
	for (unsigned int k = 3; k < imageWidth*imageHeight*cinfo.output_components; k+=4)
		dataDst[k] = 255;
